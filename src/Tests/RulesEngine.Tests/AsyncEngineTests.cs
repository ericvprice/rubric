...
namespace RulesEngine.Tests
{
    ...
    public class AsyncEngineTests
    {
        ...
        {
            var engine = new AsyncRulesEngine<TestInput, TestOutput>(new AsyncPreRule<TestInput>[] { new TestExceptionAsyncPreRule(true) }, null, null);
            var input = new TestInput();
            var output = new TestOutput();
        ...
        {
            var engine = new AsyncRulesEngine<TestInput, TestOutput>(new AsyncPreRule<TestInput>[] { new TestExceptionAsyncPreRule(false) }, null, null);
            var input = new TestInput();
            var output = new TestOutput();
        ...
        {
            var engine = new AsyncRulesEngine<TestInput, TestOutput>(null, null, new AsyncPostRule<TestOutput>[] { new TestExceptionAsyncPostRule(true) });
            var input = new TestInput();
            var output = new TestOutput();
            await Assert.ThrowsAsync<EngineHaltException>(() => engine.ApplyAsync(input, output));
            Assert.False(output.TestFlag);
        ...
        {
            var engine = new AsyncRulesEngine<TestInput, TestOutput>(null, null, new AsyncPostRule<TestOutput>[] { new TestExceptionAsyncPostRule(false) });
            var input = new TestInput();
            var output = new TestOutput();
            await Assert.ThrowsAsync<EngineHaltException>(() => engine.ApplyAsync(input, output));
            Assert.True(output.TestFlag);
        ...
        {
            var engine = new AsyncRulesEngine<TestInput, TestOutput>(null, new AsyncRule<TestInput, TestOutput>[] { new TestExceptionAsyncRule(true) }, null);
            var input = new TestInput();
            var output = new TestOutput();
            await Assert.ThrowsAsync<EngineHaltException>(() => engine.ApplyAsync(input, output));
            Assert.False(input.InputFlag);
        ...
        {
            var engine = new AsyncRulesEngine<TestInput, TestOutput>(null, new AsyncRule<TestInput, TestOutput>[] { new TestExceptionAsyncRule(false) }, null);
            var input = new TestInput();
            var output = new TestOutput();
            await Assert.ThrowsAsync<EngineHaltException>(() => engine.ApplyAsync(input, output));
            Assert.True(input.InputFlag);
