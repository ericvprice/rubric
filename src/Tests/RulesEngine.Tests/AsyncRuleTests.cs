...
namespace RulesEngine.Tests
{
    public class AsyncRuleTests
    {
        ...
        {
            var rule = new LambdaAsyncRule<TestInput, TestOutput>("test", (c, i, o) => Task.FromResult(expected), (c, i, o) => Task.CompletedTask);
            Assert.Equal(expected, await rule.DoesApply(null, null, null));
        }
        ...
        {
            var rule = new LambdaAsyncRule<TestInput, TestOutput>("test", (c, i, o) => Task.FromResult(true), (c, i, o) => Task.CompletedTask, new[] { "dep1", "dep2" }, new[] { "prv1", "prv2" });
        ...
        {
            Assert.Throws<ArgumentNullException>(() => new LambdaAsyncRule<TestInput, TestOutput>("test", null, (c, i, o) => Task.CompletedTask));
            Assert.Throws<ArgumentNullException>(() => new LambdaAsyncRule<TestInput, TestOutput>("test", (c, i, o) => Task.FromResult(true), null));
            Assert.Throws<ArgumentNullException>(() => new LambdaAsyncRule<TestInput, TestOutput>(null, (c, i, o) => Task.FromResult(true), (c, i, o) => Task.CompletedTask));
        }
