...
public class RuleEngine<TIn, TOut> : BaseRuleEngine, Rubric.Async.IRuleEngine<TIn, TOut>
    where TIn : class
    where TOut : class
{
  /// <summary>
  ...
  /// <param name="t">The cancellation token.</param>
  /// <returns>An awaitable task.</returns>
  private Task ParallelizePre(IEngineContext ctx, IEnumerable<Rules.Async.IRule<TIn>> rules, TIn i, CancellationToken t)
  {
    var cts = CancellationTokenSource.CreateLinkedTokenSource(t);
    t = cts.Token;
    return Task.WhenAll(
  ...
  /// <param name="t">The cancellation token.</param>
  /// <returns>An awaitable task.</returns>
  private Task Parallelize(IEngineContext ctx, IEnumerable<Rules.Async.IRule<TIn, TOut>> rules, TIn i, TOut o, CancellationToken t)
  {
    var cts = CancellationTokenSource.CreateLinkedTokenSource(t);
    t = cts.Token;
    return Task.WhenAll(
  ...
  /// <param name="t">The cancellation token.</param>
  /// <returns>An awaitable task.</returns>
  private Task ParallelizePost(IEngineContext ctx, IEnumerable<Rules.Async.IRule<TOut>> rules, TOut o, CancellationToken t)
  {
    var cts = CancellationTokenSource.CreateLinkedTokenSource(t);
    t = cts.Token;
    return Task.WhenAll(
