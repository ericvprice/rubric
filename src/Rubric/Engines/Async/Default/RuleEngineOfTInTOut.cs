...
#region Constructors
  ...
  public RuleEngine(Rulesets.IRuleset<TIn, TOut> ruleSet,
                    bool isParallel = false,
                    IExceptionHandler exceptionHandler = null,
                    ILogger logger = null
  ) : this(
    ...
    return Task.WhenAll(
      rules.Select(
        r => Task.Run(async () =>
        {
          try
          {
            await this.ApplyAsyncPreRule(ctx, r, i, t);
          }
          catch (Exception)
          {
            cts.Cancel();
    ...
    return Task.WhenAll(
      rules.Select(
        r => Task.Run(async () =>
        {
          try
          {
            await this.ApplyAsyncRule(ctx, r, i, o, t);
          }
          catch (Exception)
          {
            cts.Cancel();
    ...
    return Task.WhenAll(
      rules.Select(
        r => Task.Run(async () =>
        {
          try
          {
            await this.ApplyAsyncPreRule(ctx, r, o, t);
          }
          catch (Exception)
          {
            cts.Cancel();
