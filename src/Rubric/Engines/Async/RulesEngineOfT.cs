...
public class RuleEngine<T> : BaseRuleEngine, Rubric.Async.IRuleEngine<T>
    where T : class
{
  ...
  ///     Full constructor.
  /// </summary>
  /// <param name="syncRuleset">Collection of synchronous processing rules.</param>
  /// <param name="isParallel">Whether to execute rules in parallel.</param>
  /// <param name="exceptionHandler">An optional exception handler.</param>
  /// <param name="logger">A logger.</param>
  public RuleEngine(
      Rulesets.IRuleset<T> syncRules,
      bool isParallel = false,
      IExceptionHandler exceptionHandler = null,
      ILogger logger = null
  ...
  /// <inheritdoc />
  public IEnumerable<Rules.Async.IRule<T>> Rules => _rules.SelectMany(_ => _);
  ...
  private Task Parallelize(IEngineContext ctx, IEnumerable<Rules.Async.IRule<T>> rules, T i, CancellationToken t)
  {
