    ...
    where TOut : class
{
  ...
  IPreRuleBuilder<TIn, TOut> WithPredicate(Func<IEngineContext, TIn, Task<double>> predicate);
  ...
  /// <param name="predicate">The predicate function.</param>
  /// <returns>A fluent continuation.</returns>
  IPreRuleBuilder<TIn, TOut> WithPredicate(Func<IEngineContext, TIn, CancellationToken, Task<double>> predicate);
