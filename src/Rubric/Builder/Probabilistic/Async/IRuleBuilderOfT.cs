  ...
  where T : class
{
  ...
  IRuleBuilder<T> WithPredicate(Func<IEngineContext, T, Task<double>> predicate);
  ...
  /// <param name="predicate">The predicate function.</param>
  /// <returns>A fluent continuation.</returns>
  IRuleBuilder<T> WithPredicate(Func<IEngineContext, T, CancellationToken, Task<double>> predicate);
